...
Agents with non-system skills automatically get a Capsule sandbox -- no manual sandbox configuration needed. The sandbox is an isolated Firecracker microVM provisioned using the agent's `sandbox_template`. Configure sandbox behavior via the `sandbox_template` and `sandbox_envs` fields above.
...
Route a message to a specific agent by setting `agentName` on `SendOptions`. To discover the names, call [`GET /v1/agents`](/api-reference/agents/list) directly — `session.agents` does not populate against the current backend (see the note on that page).
