...
```swift
// connect() populates the agent list
for agent in session.agents where agent.isEnabled {
    print("\(agent.name): \(agent.displayName ?? agent.name)")
}

// Route a message to one of them
