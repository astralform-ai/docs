...
<Warning>
  For **GitHub**, neither **Disconnect** (unbind this agent from its GitHub account) nor **Uninstall** (remove the connector entirely) removes the App from your account or organization. That is yours to do on GitHub, and the confirmation dialog links you there. If other agents in your workspace answer for the same account, the dialog says so — removing the App on GitHub stops all of them. See [GitHub](/dashboard/github#disconnecting).
</Warning>
