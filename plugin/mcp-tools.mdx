...
These tools manage the agent's **conversation model** — the model it uses when a request
doesn't name one.
...
<Note>
  Provider API keys are held **per team**, in the Model Providers vault — not per agent.
  Connect the key with [`astralform_set_provider_key`](#provider-keys) *before* setting a
  model; `astralform_set_llm_config` refuses a provider whose key isn't connected, rather
  than saving a model that would fail at request time.
</Note>
...
Claude will call `astralform_set_provider_key` first, then `astralform_set_llm_config` with
the provider and model.

Clearing the model leaves the agent with no default: every request must then name a model
